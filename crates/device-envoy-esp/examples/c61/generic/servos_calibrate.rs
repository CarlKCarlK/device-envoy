...
//!   red -> 5V, brown -> GND, yellow -> signal (GPIO10 / GPIO11)
...
async fn inner_main(_spawner: Spawner) -> Result<Infallible> {
    ...
    let servo_b = ServoB::new(&ledc, p.GPIO11)?;
...
fn set_both(servo_a: &ServoEsp, servo_b: &ServoEsp, degrees: u16) {
    info!(
        "servos_calibrate: set both servos to {} degrees (GPIO10, GPIO11)",
        degrees
    );
