...
/// Run the shared clock UI state machine.
...
{
    let mut state = ClockUiState::HoursMinutes {
    ...
    OnEventFuture: core::future::Future<Output = Result<(), E>>,
{
    ...
    loop {
        match select(button.wait_for_press_duration(), clock_sync.wait_for_tick()).await {
            Either::First(PressDuration::Short) if speed.to_bits() == REAL_TIME_SPEED.to_bits() => {
            ...
            Either::First(PressDuration::Long) => return Ok(ClockUiState::EditOffset),
            Either::Second(tick) => {
    ...
    OnEventFuture: core::future::Future<Output = Result<(), E>>,
{
    ...
    loop {
        match button.wait_for_press_duration().await {
            PressDuration::Short => {
                let day_minutes = ONE_DAY.as_secs() / 60;
                assert!(day_minutes <= i32::MAX as u64);
                let day_minutes = day_minutes as i32;
                offset_minutes =
                    (offset_minutes + EDIT_OFFSET_STEP_MINUTES).rem_euclid(day_minutes);
                clock_sync.set_offset_minutes(offset_minutes);
                let (hours, minutes, _) = h12_m_s(&clock_sync.now_local());
                on_event(ClockUiEvent::RenderHoursMinutesEdit { hours, minutes }).await?;
            }
            PressDuration::Long => {
                flash.save(&offset_minutes)?;
                return Ok(ClockUiState::HoursMinutes {
                    speed: REAL_TIME_SPEED,
                });
            }
