...
async fn inner_main(spawner: Spawner) -> core::result::Result<Infallible, MainError> {
    ...
    let mut button = ButtonWatch::new(p.GPIO0, PressedTo::Ground, spawner).await?;
    ...
    let stack = wifi_auto
        .connect(
            &mut *button,
            async |wifi_auto_event| -> core::result::Result<(), MainError> {
    ...
    let mut dns = DnRuntime::new(DNS_HOSTNAME, async || {
