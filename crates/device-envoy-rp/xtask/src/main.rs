...
fn check_board(board: Board, arch: Arch, caps: CapabilitySet) -> ExitCode {
    ...
    let example_features = build_example_features(board, caps);
    let example_features_no_wifi = build_example_features(board, CapabilitySet::empty());
    ...
    no_wifi_examples.par_iter().for_each(|example| {
        if !run_command(Command::new("cargo").current_dir(&workspace_root).args([
            ...
            target,
            "--features",
            example_features_no_wifi.as_str(),
            "--no-default-features",
        ])) {
            failures
                .lock()
...
fn check_all() -> ExitCode {
    ...
    let example_features_no_wifi = build_example_features(board_pico2, no_capabilities);
    let example_features_wifi_pico2 = build_example_features(board_pico2, wifi_capabilities);
    let example_features_wifi_pico1 = build_example_features(board_pico1, wifi_capabilities);
        ...
        s.spawn(|_| {
            ...
            examples.par_iter().for_each(|example| {
                if !run_command(Command::new("cargo").current_dir(&workspace_root).args([
                    ...
                    target_pico2,
                    "--features",
                    example_features_wifi_pico2.as_str(),
                    "--no-default-features",
                ])) {
        ...
        s.spawn(|_| {
            ...
            examples.par_iter().for_each(|example| {
                if !run_command(Command::new("cargo").current_dir(&workspace_root).args([
                    ...
                    target_pico1,
                    "--features",
                    example_features_wifi_pico1.as_str(),
                    "--no-default-features",
                ])) {
...
fn check_readme_example() -> ExitCode {
    ...
    let examples_crate_root = crate_root.join("../device-envoy-examples-rp");
    let generated_example_path = examples_crate_root.join("examples/__readme_example_generated.rs");
    ...
    let target = arch.target(board);
    let wifi_capabilities = CapabilitySet::with_capabilities(&[Capability::Wifi]);
    let features = build_example_features(board, wifi_capabilities);
...
fn check_examples() -> ExitCode {
    ...
    let target_pico1 = arch.target(board_pico1);
    let wifi_capabilities = CapabilitySet::with_capabilities(&[Capability::Wifi]);
    let features_no_wifi = build_example_features(board_pico2, CapabilitySet::empty());
    let features_wifi_pico2 = build_example_features(board_pico2, wifi_capabilities);
    let features_wifi_pico1 = build_example_features(board_pico1, wifi_capabilities);
...
fn build_example(name: &str, board: Board, arch: Arch, capability_set: CapabilitySet) -> ExitCode {
    let workspace_root = workspace_root();
    let target = arch.target(board);
    let features = build_example_features(board, capability_set);
    println!(
        "{}",
...
fn discover_examples(workspace_root: &Path) -> Vec<ExampleInfo> {
    let examples_dir = workspace_root.join("crates/device-envoy-examples-rp/examples");
...
fn build_example_features(board: Board, capability_set: CapabilitySet) -> String {
    let mut features = vec![match board {
        Board::Pico1 => "embedded",
        Board::Pico2 => "embedded-pico2",
    }];
    if capability_set.contains(Capability::Wifi) {
        features.push(Capability::Wifi.feature_name());
    }
    features.join(",")
}
