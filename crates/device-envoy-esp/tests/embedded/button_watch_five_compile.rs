...
async fn inner_main(spawner: Spawner) -> device_envoy_esp::Result<core::convert::Infallible> {
    ...
    let button_watch0 = ButtonWatch0::new(p.GPIO0, PressedTo::Ground, spawner)?;
    let button_watch1 = ButtonWatch1::new(p.GPIO1, PressedTo::Ground, spawner)?;
    let button_watch2 = ButtonWatch2::new(p.GPIO2, PressedTo::Ground, spawner)?;
    let button_watch3 = ButtonWatch3::new(p.GPIO3, PressedTo::Ground, spawner)?;
    let button_watch4 = ButtonWatch4::new(p.GPIO4, PressedTo::Ground, spawner)?;
