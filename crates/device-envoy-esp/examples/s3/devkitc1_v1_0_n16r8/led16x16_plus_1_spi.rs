...
//! Mixed backend demo for `esp32-s3-devkitc-1-v1.0-n16r8` (ESP32-S3): 16x16 panel via SPI and one NeoPixel-style (WS2812) LED via RMT.
//!
//! Wiring:
//! - GPIO2: 16x16 NeoPixel-style (WS2812) panel data-in (SPI MOSI)
//! - GPIO48: built-in NeoPixel-style (WS2812) LED (RMT)
...
async fn inner_main(spawner: Spawner) -> Result<Infallible> {
    ...
    info!(
        "led16x16_plus_1_spi: panel on GPIO{} via SPI2, strip1 LED on GPIO{} via RMT (esp32-s3-devkitc-1-v1.0-n16r8)",
        PANEL_16X16_PIN_NUM, LED_STRIP1_PIN_NUM
    );

    let led16x16_dual_spi = Led16x16DualSpi::new(p.GPIO2, p.SPI2, spawner)?;
    let led_strip1_rmt = LedStrip1Rmt::new(p.GPIO48, rmt80.channel1, spawner)?;
