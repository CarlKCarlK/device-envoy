...
//! [`tga565!`](crate::cyd::display::tga565) and
//! [`tga565_mask!`](crate::cyd::display::tga565_mask) macros to avoid spelling
//! the `N`/`MASK_N` const arguments by hand.
//!
...
///
/// `N` must equal `W * H`; the [`tga565!`](crate::cyd::display::tga565) macro
/// computes it for you. 32-bit (BGRA) sources are accepted but their alpha is
/// ignored — use [`Image565Mask`] when transparency matters.
...
/// [`tga565_mask!`](crate::cyd::display::tga565_mask) macro computes both. For 24-bit sources
...
impl<const W: usize, const H: usize, const N: usize> Image565Fixed<W, H, N> {
    ...
    /// accepted subset. Prefer [`tga565!`](crate::cyd::display::tga565) at call sites.
    ...
    pub const fn view(&'static self) -> super::Image565View {
        super::Image565View::new_cropped(
            &self.pixels,
            W as u32,
    ...
    Image565Mask<W, H, N, MASK_N>
{
    ...
    /// [`tga565_mask!`](crate::cyd::display::tga565_mask) at call sites.
    pub const fn from_tga(bytes: &[u8]) -> Self {
...
/// # use device_envoy_core::cyd::display::{Image565Fixed, tga565};
...
macro_rules! __cyd_tga565 {
    ($path:expr, $width:expr, $height:expr) => {
        $crate::cyd::display::Image565Fixed::<$width, $height, { $width * $height }>::from_tga(
            include_bytes!($path),
        )
    };
}
...
/// # use device_envoy_core::cyd::display::{Image565Mask, tga565_mask};
...
macro_rules! __cyd_tga565_mask {
    ($path:expr, $width:expr, $height:expr) => {{
        type Image = $crate::cyd::display::Image565Mask<
            $width,
            $height,
            { $width * $height },
            { ($width * $height + 7) / 8 },
...
/// # use device_envoy_core::cyd::display::{Image565Mask, tga565_magenta_mask};
...
/// // `ignore`: `include_bytes!` resolves at compile time even under `no_run`,
/// // so this needs a real `hours.small.tga` on disk to build.
/// # use device_envoy_core::cyd::display::{Image565Mask, tga565_white_mask};
