...
- The build script rasterizes the SVG files into static RGB TGA backgrounds in `OUT_DIR`.
...
The sample dynamic text and developer-only guides are grouped under:
- preview-values
- preview-values-latency
- preview-values-stats
- developer-guides

The build script removes those groups when creating a static background bitmap.
